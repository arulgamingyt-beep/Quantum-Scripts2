...
---------------------------------------------
-- WEAPON SHOPS (Fixed Locations)
---------------------------------------------
local ShopProps = {}
local ShopBlips = {}
local currentShopJob = nil
local currentShopLabel = nil

local function CreateShopBlip(shopData, coords)
    if not shopData.blip or not shopData.blip.enabled then return end
    local blip = Citizen.InvokeNative(0x554D9D53F696D002, 1664425300, coords.x, coords.y, coords.z)
    SetBlipSprite(blip, GetHashKey(shopData.blip.sprite), true)
    SetBlipScale(blip, shopData.blip.scale)
    Citizen.InvokeNative(0x662D364ABF16DE2F, blip, GetHashKey(shopData.blip.color))
    SetBlipName(blip, shopData.label)
    return blip
end
...
local function IsBoss(jobName)
    local PlayerData = RSGCore.Functions.GetPlayerData()
    if PlayerData and PlayerData.job and PlayerData.job.name == jobName then
        if PlayerData.job.isboss or PlayerData.job.isBoss or PlayerData.job.grade.level >= 3 then
            return true
        end
    end
    return false
...
-- Hire Employee Menu
local function OpenHireMenu()
    local input = lib.inputDialog('Hire Employee', {
        { type = 'number', label = 'Player ID', description = 'Enter the player server ID', required = true },
        { type = 'select', label = 'Grade', description = 'Select employee grade', required = true, options = {
            { value = '0', label = 'Apprentice Smith' },
            { value = '1', label = 'Gunsmith' },
            { value = '2', label = 'Senior Gunsmith' },
            { value = '3', label = 'Master Smith (Boss)' },
        }},
    })
    
    if input then
        TriggerServerEvent('rsg-weaponcomp:server:hireEmployee', currentShopJob, tonumber(input[1]), tonumber(input[2]))
    end
end

-- Fire Employee Menu
local function OpenFireMenu()
    RSGCore.Functions.TriggerCallback('rsg-weaponcomp:server:getEmployees', function(employees)
        if not employees or #employees == 0 then
            lib.notify({ title = 'No Employees', description = 'There are no employees to fire', type = 'error' })
            return
        end
        ...
        for _, emp in ipairs(employees) do
            table.insert(options, {
                title = emp.name,
                description = 'Grade: ' .. emp.gradeName,
                icon = 'user',
                onSelect = function()
                    ...
                    if confirm == 'confirm' then
                        TriggerServerEvent('rsg-weaponcomp:server:fireEmployee', emp.identifier)
                    end
                end
            })
        end
        ...
        lib.registerContext({
            id = 'weaponshop_fire_menu',
            title = 'Fire Employee',
            menu = 'weaponshop_boss_menu',
            options = options
        })
        lib.showContext('weaponshop_fire_menu')
    end, currentShopJob)
end
...
local function OpenSalaryMenu()
    RSGCore.Functions.TriggerCallback('rsg-weaponcomp:server:getEmployees', function(employees)
        if not employees or #employees == 0 then
            lib.notify({ title = 'No Employees', description = 'There are no employees', type = 'error' })
            return
        end
        ...
        for _, emp in ipairs(employees) do
            table.insert(options, {
                title = emp.name,
                description = 'Current Grade: ' .. emp.gradeName .. ' | Payment: $' .. emp.payment,
                icon = 'money-bill',
                onSelect = function()
                    local input = lib.inputDialog('Change Grade - ' .. emp.name, {
                        { type = 'select', label = 'New Grade', required = true, options = {
                            { value = '0', label = 'Apprentice Smith ($15)' },
                            { value = '1', label = 'Gunsmith ($30)' },
                            { value = '2', label = 'Senior Gunsmith ($50)' },
                            { value = '3', label = 'Master Smith ($80)' },
                        }},
                    })
                    if input then
                        TriggerServerEvent('rsg-weaponcomp:server:changeGrade', emp.identifier, tonumber(input[1]))
                    end
                end
            })
        end
        ...
        lib.registerContext({
            id = 'weaponshop_salary_menu',
            title = 'Salary Management',
            menu = 'weaponshop_boss_menu',
            options = options
        })
        lib.showContext('weaponshop_salary_menu')
    end, currentShopJob)
end

-- Analysis Report Menu
local function OpenAnalysisMenu()
    RSGCore.Functions.TriggerCallback('rsg-weaponcomp:server:getShopAnalysis', function(data)
        if not data then
            lib.notify({ title = 'Error', description = 'Could not fetch analysis data', type = 'error' })
            return
        end
        ...
        local options = {
            {
                ...
                description = tostring(data.totalEmployees),
                icon = 'users',
            },
            {
                title = 'Total Sales Today',
                description = '$' .. tostring(data.salesToday),
                icon = 'chart-line',
            },
            {
                title = 'Total Revenue (All Time)',
                description = '$' .. tostring(data.totalRevenue),
                icon = 'dollar-sign',
            },
            {
                title = 'Weapons Customized',
                description = tostring(data.weaponsCustomized),
                icon = 'wrench',
            },
        }
        
        lib.registerContext({
            id = 'weaponshop_analysis_menu',
            title = 'Shop Analysis Report',
            menu = 'weaponshop_boss_menu',
            options = options
        })
        lib.showContext('weaponshop_analysis_menu')
    end, currentShopJob)
end

-- Main Boss Menu
local function OpenBossMenu(jobName, shopLabel)
    currentShopJob = jobName
    currentShopLabel = shopLabel
    
    lib.registerContext({
        id = 'weaponshop_boss_menu',
        title = 'BOSS MENU',
        options = {
            {
                title = 'Hire Employee',
                ...
                icon = 'user-plus',
                onSelect = OpenHireMenu
            },
            {
                title = 'Fire Employee',
                description = 'Fire an existing employee',
                icon = 'user-minus',
                onSelect = OpenFireMenu
            },
            {
                title = 'Salary Management',
                description = 'Manage employee grades and salaries',
                icon = 'money-bill-wave',
                onSelect = OpenSalaryMenu
            },
            {
                title = 'Analysis Report',
                description = 'View shop statistics and reports',
                icon = 'chart-bar',
                onSelect = OpenAnalysisMenu
            },
        }
    })
    lib.showContext('weaponshop_boss_menu')
end

local function SetupWeaponShops()
    for shopId, shopData in pairs(Config.WeaponShops) do
        if shopData.enabled then
            local jobName = shopData.job
            local shopLabel = shopData.label
            
            -- Create blip
            if shopData.blip and shopData.blip.enabled then
                local blipCoords = shopData.prop.coords
                ShopBlips[shopId] = CreateShopBlip(shopData, vector3(blipCoords.x, blipCoords.y, blipCoords.z))
            end

            -- Spawn prop or use existing game prop
            if shopData.prop then
                local propCoords = shopData.prop.coords
                local obj = nil
                
                -- Only spawn prop if enabled is true or not set
                if shopData.prop.enabled ~= false then
                    local m = joaat(shopData.prop.model)
                    RequestModel(m)
                    while not HasModelLoaded(m) do Wait(1) end
                    obj = CreateObject(m, propCoords.x, propCoords.y, propCoords.z, false, true, true)
                    SetEntityHeading(obj, propCoords.w)
                    SetEntityCoordsNoOffset(obj, propCoords.x, propCoords.y, propCoords.z, false, false, false)
                    FreezeEntityPosition(obj, true)
                    SetEntityCollision(obj, false, false)
                    ShopProps[shopId] = obj
                    
                    -- Add target to spawned prop
                    exports.ox_target:addLocalEntity(obj, {
                        {
                            name = 'shop_weapon_custom_' .. shopId,
                            icon = 'fas fa-wrench',
                            label = locale('cl_lang_14'),
                            onSelect = function()
                                local wHash = GetPedCurrentHeldWeapon(PlayerPedId())
                                ...
                                local fakePropId = 'shop_' .. shopId
                                SpawnedProps[fakePropId] = { obj = obj }
                                TriggerEvent('rsg-weaponcomp:client:startcustom', fakePropId, wHash, serial, weaponName)
                            end,
                            distance = 2.0
                        },
                    })
                else
                    -- Use sphere zone for existing game prop (no spawn)
                        ...
                        options = {
                            {
                                ...
                                onSelect = function()
                                    ...
                                    -- Find nearest prop for customization
                                    local nearestProp = GetClosestObjectOfType(propCoords.x, propCoords.y, propCoords.z, 2.0, joaat(shopData.prop.model), false, false, false)
                                    local fakePropId = 'shop_' .. shopId
                                    SpawnedProps[fakePropId] = { obj = nearestProp ~= 0 and nearestProp or nil }
                                    TriggerEvent('rsg-weaponcomp:client:startcustom', fakePropId, wHash, serial, weaponName)
                                end,
                                distance = 2.0
                            },
            ...
            -- Boss menu using ox_target (boss only)
            if shopData.boss and shopData.boss.enabled then
                local bossCoords = shopData.boss.coords
                exports.ox_target:addSphereZone({
                    coords = bossCoords,
                    radius = 1.5,
                    debug = false,
                    options = {
                        {
                            ...
                            canInteract = function()
                                return IsBoss(jobName)
                            end,
                            onSelect = function()
                                OpenBossMenu(jobName, shopLabel)
                            end,
                            distance = 2.0
                        },
                    }
            ...
            -- Stash using ox_target (ALL job grades can access)
            if shopData.stash and shopData.stash.enabled then
                local stashCoords = shopData.stash.coords
                local stashId = shopId
                exports.ox_target:addSphereZone({
                    coords = stashCoords,
                    radius = 1.5,
                    debug = false,
                    options = {
                        {
                            ...
                            onSelect = function()
                                TriggerServerEvent('rsg-weaponcomp:server:openStash', stashId)
                            end,
                            distance = 2.0
                        },
                    }
...
-- Cleanup shops on resource stop
AddEventHandler('onResourceStop', function(resource)
    if resource ~= GetCurrentResourceName() then return end
    for shopId, obj in pairs(ShopProps) do
